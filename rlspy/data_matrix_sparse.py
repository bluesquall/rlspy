#!/usr/bin/env python
"""
Data matrix (sparse) form for recursive least squares estimation
================================================================
...
def priorgain(P, A, V):
    ...
    return sp.sparse.linalg.spsolve(APATpV.T, PAT.T).T
#    for spsolve, rhs must apparently be a vector and it isn't here...
#       this probably means we need to just do row-by-row updates
...
class Estimator(object):
    ...
    def __init__(self, x0, P0):
        ...
        self.x = sp.sparse.csr_matrix(x0)
        self.P = sp.sparse.csr_matrix(P0)
        # identity matrix same size as P for convenience
        self.I = sp.sparse.identity(x0.shape[0]) 
...
#        A = A.reshape(1, -1) # TODO consider np.atleast2d
    ...
    def consecutive_update(self, A, b, v):
        ...
        [self.single_update(Ar, br, v) for Ar, br in itertools.izip(A, b)]
